...
def split_text_into_chunks(text: str, max_tokens: int) -> list[str]:
    ...
    sentences_token_amounts = [
        (sentence, len(tokenize_text(sentence))) for sentence in sentences
    ]
    ...
    for idx, (sentence, size) in enumerate(sentences_token_amounts):
        ...
        _, next_size = (
            sentences_token_amounts[idx + 1]
            if idx < len(sentences_token_amounts) - 1
            else ("", -1)
        )
        ...
        if next_size == -1:
            ...
            accumulative_size + next_size > max_tokens
            and accumulative_size <= max_tokens
        ):
            chunks.append(accumulative_chunk)
            accumulative_chunk = ""
            accumulative_size = 0
